...
export const addCar = async (req, res) => {
    try {
        ...
        const uploadImages = await Promise.all(
            imageFiles.map(file => uploadToImageKit(file.path, file.originalname, '/cars'))
        )
    ...
    try {
        ...
        if (req.files && req.files.length > 0) {
            ...
            const newImages = await Promise.all(
                req.files.map(file => uploadToImageKit(file.path, file.originalname, '/cars'))
            )
        ...
        if (car.images.length === 0) {
            return res.json({success: false, message: "Car must have at least 1 image"})
        }

        // ✅ Step 3 — update other fields if provided
        if (carData) {
            const updates = JSON.parse(carData)
            const allowedFields = ['brand', 'model', 'year', 'category', 'seating_capacity',
